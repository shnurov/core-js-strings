...
function concatenateStrings(value1, value2) {
  const str = ''.concat(value1, value2);
  return str;
}
...
function removeFirstOccurrences(str, value) {
  ...
  if (str.includes(value)) {
    ...
    const lengthx = value.length;
    return str.slice(0, index) + str.slice(index + lengthx);
  }
  return str;
}
...
function formatTime(minutes, seconds) {
  const a = `${minutes}:`.padStart(3, '0');
  const b = `${seconds}`.padStart(2, '0');
  return a + b;
}
...
function countVowels(str) {
  const list = 'aeiouyAYEIOU';
  let res = 0;
  for (let i = 0; i < str.length; i += 1) {
    if (list.includes(str[i])) res += 1;
  }
  return res;
}
...
function isPalindrome(str) {
  ...
  const regExp = /\?|,|!|&|\$/g;
  const str2 = str1.replace(regExp, '');
  let res = '';
  for (let i = str2.length - 1; i >= 0; i -= 1) {
    res += str2[i];
  }
  return str2.includes(res);
}
...
function reverseWords(str) {
  ...
  for (let k = 0; k < arr.length; k += 1) {
    let res = ' ';
    for (let i = arr[k].length - 1; i >= 0; i -= 1) {
      res += arr[k][i];
    }
    sentence += res;
  }
...
function invertCase(str) {
  let res = '';
  const list1 = 'ZAQWSXCDERFVBGTYHNMJUIKLOP';
  const list2 = 'zaqwsxcderfvbgtyhnmjuiklop';

  for (let i = 0; i < str.length; i += 1) {
    if (list1.includes(str[i])) {
      res += str[i].toLowerCase();
    } else if (list2.includes(str[i])) {
...
function encodeToRot13(str) {
  ...
  const list2 = 'abcdefghijklmnopqrstuvwxyzabcdefghijklmnopqrstuvwxyz';
  let res = '';
  for (let i = 0; i < str.length; i += 1) {
    if (list1.includes(str[i])) {
      const ind = list1.indexOf(str[i]);
      res += list1[ind + 13];
    } else if (list2.includes(str[i])) {
      const ind = list2.indexOf(str[i]);
      res += list2[ind + 13];
    } else res += str[i];
  }
  return res;
